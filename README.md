...
- **Widget-source resolution** (`sources.rs`): the three cross-cutting couplings (Ryu's
  managed provider router for Composio, the agent runner for agent widgets, and the
  SSRF-guarded external fetch for HTTP widgets) are inverted through the `DashboardsHost` trait, so the crate has
...
<a href="https://github.com/amajorai/ryu-dashboards/stargazers">
  <picture>
