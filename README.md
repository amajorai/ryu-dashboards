...
implementation stays in-crate. There is no `apps-store/dashboards/ui/plugin.json`, so
the app does **not** participate in the `companion_fixtures_match_their_package_manifests`
byte-identity test (that test only covers companion apps that embed an HTML UI).
