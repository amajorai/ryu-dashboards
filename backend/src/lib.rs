...
pub(crate) mod testutil {
    ...
    /// (Gateway/Composio, agent runner, SSRF fetch) return canned values and record
    /// their calls, so `sources::resolve` can be driven with ZERO network for the
    /// host-routed source kinds (Agent, HTTP, and — via a closed `gateway_url` — the
    /// Composio error path).
    pub struct FakeHost {
    ...
    impl Default for FakeHost {
        fn default() -> Self {
            Self {
                // Closed loopback port: the Composio path's real reqwest POST fails
                // fast (connection refused) so its error branch is exercised without
                // any live server, and never reaches an external host.
