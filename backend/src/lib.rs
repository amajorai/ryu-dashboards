...
//! calls the source resolver needs — the managed provider router (Composio widgets),
...
use ryu_app_events::{ManagedProviderCall, ManagedProviderResponse};
...
/// in `apps-store/dashboards/manifest.json`.
pub(crate) const DASHBOARDS_PLUGIN_ID: &str = "@ryu/dashboards";

/// A widget's source stopped resolving. Declared in the manifest's
/// `contributes.hook_events`; an undeclared id is rejected at emit.
pub(crate) const EVENT_WIDGET_FAILED: &str = "@ryu/dashboards#widget.failed";
/// A widget that had been failing resolved again.
pub(crate) const EVENT_WIDGET_RECOVERED: &str = "@ryu/dashboards#widget.recovered";
...
/// [`DashboardEngine`].
///
/// Only the *expensive* source kinds need the host: managed provider calls (through
/// the Core → Gateway bridge), Agent (through the agent runner), and arbitrary HTTP (through the
...
pub trait DashboardsHost: Send + Sync {
    /// Execute a provider-neutral operation through Ryu's managed provider router.
    /// Provider credentials and organization billing remain outside this satellite.
...
pub enum WidgetSource {
    ...
    Static {
        /// The value to render, exactly as the widget kind expects it.
        #[serde(default)]
        data: Value,
    },
    ...
    CoreEndpoint {
        /// One of the names listed by `GET /api/dashboards/catalog`. Anything
        /// else is rejected — this is an allowlist, not a URL.
        endpoint: String,
        /// A dotted path into the JSON response, e.g. `clients.0.name`.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        selector: Option<String>,
    },
    /// A website monitor's latest check result.
    Monitor {
        /// The monitor whose latest check result this widget shows.
        monitor_id: String,
    },
    /// Run a saved workflow and read a key from its output map.
    Workflow {
        /// The saved workflow to run. A plain id — no slashes or query chars.
        workflow_id: String,
        /// Inputs passed to the workflow on each run.
        #[serde(default)]
        input: std::collections::HashMap<String, String>,
        /// Which key of the workflow's output map to render. Omit for the whole map.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        output_key: Option<String>,
    },
    /// Execute a Composio action through the Gateway.
    Composio {
        /// The Composio action id. A plain id — no slashes or query chars.
        action: String,
        /// Arguments passed to the action on each run.
        #[serde(default)]
        args: Value,
    },
    ...
    Http {
        /// The endpoint to poll. Must be publicly routable — loopback and
        /// private ranges are refused at resolve time.
        url: String,
        /// A dotted path into the JSON response, e.g. `data.items`.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        selector: Option<String>,
    ...
    Agent {
        /// The agent to run.
        agent_id: String,
        /// The prompt sent on each refresh. Ask for JSON the widget kind can render.
        prompt: String,
    },
}
...
pub struct GridLayout {
    /// Column offset from the left edge of the 12-column grid.
    pub x: u32,
    /// Row offset from the top of the grid.
    pub y: u32,
    /// Width in grid columns (the grid is 12 wide).
    pub w: u32,
    /// Height in grid rows.
    pub h: u32,
}
...
/// Shared test scaffolding (a fake [`DashboardsHost`] + temp-store engine builders)
/// reused by every module's `#[cfg(test)] mod tests` via `crate::testutil`. Kept in
/// one place so the fake host contract lives once, not copied per module.
#[cfg(test)]
pub(crate) mod testutil {
    use super::*;
    use std::sync::Mutex;

    /// A fully in-memory [`DashboardsHost`]: the three inverted couplings
    /// (managed provider, agent runner, SSRF fetch) return canned values and record
    /// their calls, so `sources::resolve` can be driven with ZERO network for the
    /// host-routed source kinds.
    pub struct FakeHost {
        pub provider_reply: Mutex<Result<ManagedProviderResponse, String>>,
        pub provider_calls: Mutex<Vec<ManagedProviderCall>>,
        pub agent_reply: Mutex<Result<String, String>>,
        pub fetch_reply: Mutex<Result<(u16, String), String>>,
        pub agent_calls: Mutex<Vec<(String, String, String)>>,
        pub fetch_calls: Mutex<Vec<(String, Vec<(String, String)>)>>,
    }

    impl Default for FakeHost {
        fn default() -> Self {
            Self {
                provider_reply: Mutex::new(Err("provider router unavailable".to_owned())),
                provider_calls: Mutex::new(Vec::new()),
                agent_reply: Mutex::new(Ok(String::new())),
                fetch_reply: Mutex::new(Ok((200, "{}".to_string()))),
    ...
    #[async_trait]
    impl DashboardsHost for FakeHost {
        async fn call_provider(
            &self,
            call: ManagedProviderCall,
        ) -> Result<ManagedProviderResponse, String> {
            self.provider_calls.lock().unwrap().push(call);
            self.provider_reply.lock().unwrap().clone()
        }
        async fn agent_run(
            &self,
            agent_id: &str,
            ...
            self.agent_reply.lock().unwrap().clone()
        }
        async fn guarded_fetch(
            &self,
            url: &str,
            ...
            self.fetch_reply.lock().unwrap().clone()
        }
    }

    /// A store backed by a fresh temp `dashboards.db` (unique per call).
    pub fn temp_store() -> DashboardStore {
        let dir = std::env::temp_dir().join(format!("ryu-dash-test-{}", uuid::Uuid::new_v4()));
        DashboardStore::open(dir.join("dashboards.db")).expect("open temp store")
    }

    /// An engine over a fresh temp store with the supplied host.
    pub fn engine_with(host: Arc<dyn DashboardsHost>) -> DashboardEngine {
        DashboardEngine::new(temp_store(), reqwest::Client::new(), host)
    }

    /// An engine over a fresh temp store with a default [`FakeHost`].
    pub fn test_engine() -> DashboardEngine {
...
mod tests {
    ...
    fn legacy_widget_row_without_canvas_still_parses() {
        ...
        assert!(
            ...
            "skip_serializing_if keeps it out"
        );
    }
    ...
    fn legacy_dashboard_row_without_view_mode_still_parses() {
        ...
        assert!(
            d.view_mode.is_none(),
            "absent view_mode ⇒ None (treated as grid)"
        );
        ...
        let back: Dashboard =
            serde_json::from_value(serde_json::to_value(&canvas).unwrap()).unwrap();
        assert_eq!(back.view_mode.as_deref(), Some("canvas"));
    }

    #[test]
    fn min_and_default_intervals_per_source_kind() {
        let ce = WidgetSource::CoreEndpoint {
            endpoint: "agents".into(),
            selector: None,
        };
        assert_eq!(ce.min_interval_secs(), 5);
        assert_eq!(ce.default_interval_secs(), 15);

        let http = WidgetSource::Http {
            url: "https://x".into(),
            selector: None,
            headers: Default::default(),
        };
        assert_eq!(http.min_interval_secs(), 30);
        assert_eq!(http.default_interval_secs(), 60);

        let composio = WidgetSource::Composio {
            action: "a".into(),
            args: Value::Null,
        };
        assert_eq!(composio.min_interval_secs(), 30);
        assert_eq!(composio.default_interval_secs(), 120);

        // Static never refreshes ⇒ effectively-infinite floors.
        let s = WidgetSource::Static { data: Value::Null };
        assert_eq!(s.min_interval_secs(), u64::MAX);
        assert_eq!(s.default_interval_secs(), u64::MAX);
        assert!(!s.is_expensive());
    }

    #[test]
    fn unparseable_interval_falls_back_to_default() {
        // A garbage humantime string is ignored ⇒ the per-kind default is used.
        let w = widget_with(
            WidgetSource::CoreEndpoint {
                endpoint: "agents".into(),
                selector: None,
        ...
        assert_eq!(w.effective_interval_secs(), 15);

        // A blank/whitespace interval is also treated as unset.
        let w2 = widget_with(
            WidgetSource::CoreEndpoint {
                endpoint: "agents".into(),
                selector: None,
        ...
        assert_eq!(w2.effective_interval_secs(), 15);
    }

    #[test]
    fn parseable_interval_above_floor_is_kept() {
        // 5m on a cheap source (floor 5s) is kept verbatim (300s).
        let w = widget_with(
            WidgetSource::CoreEndpoint {
                endpoint: "agents".into(),
                selector: None,
        ...
        assert_eq!(w.effective_interval_secs(), 300);
    }

    #[tokio::test]
    async fn replace_widgets_rejects_non_array() {
        let engine = testutil::test_engine();
        let err = replace_widgets(&engine, "d1", &json!({ "not": "an array" }))
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn replace_widgets_rejects_bad_core_endpoint() {
        let engine = testutil::test_engine();
        let widgets = json!([{
            "kind": "stat",
            "source": { "type": "core_endpoint", "endpoint": "secrets" }
        }]);
        let err = replace_widgets(&engine, "d1", &widgets).await.unwrap_err();
        assert!(
            err.to_string().contains("not an allowed core_endpoint"),
            "{err}"
        );
        // Rejected batch leaves nothing behind (all-or-nothing apply).
        assert!(engine.store.list_widgets("d1").await.unwrap().is_empty());
    }

    #[tokio::test]
    async fn replace_widgets_rejects_malformed_widget() {
        let engine = testutil::test_engine();
        // Missing the required `kind`/`source` fields ⇒ serde failure for widget #0.
        let widgets = json!([{ "title": "no kind" }]);
        let err = replace_widgets(&engine, "d1", &widgets).await.unwrap_err();
        assert!(err.to_string().contains("widget #0 is invalid"), "{err}");
    }

    #[tokio::test]
    async fn replace_widgets_assigns_ids_binds_and_replaces() {
        let engine = testutil::test_engine();
        // Seed an existing widget that must be dropped by the replace.
        let existing = widget_with(WidgetSource::Static { data: Value::Null }, None);
        let mut existing = existing;
        existing.id = "old".into();
        existing.dashboard_id = "d1".into();
        engine.store.upsert_widget(&existing).await.unwrap();

        let widgets = json!([
            {
                "kind": "stat",
                "source": { "type": "static", "data": 1 },
                // caller sends cached state + no id: id is generated, cache cleared.
                "last_value": 99,
                "last_error": "stale"
            },
            {
                "id": "keepme",
                "kind": "text",
                "source": { "type": "static", "data": "x" }
            }
        ]);
        let n = replace_widgets(&engine, "d1", &widgets).await.unwrap();
        assert_eq!(n, 2);

        let stored = engine.store.list_widgets("d1").await.unwrap();
        assert_eq!(stored.len(), 2, "old widget replaced, two new ones present");
        assert!(stored.iter().all(|w| w.dashboard_id == "d1"));
        assert!(stored.iter().all(|w| w.last_value.is_none()));
        assert!(stored.iter().all(|w| w.last_error.is_none()));
        // The generated id follows the wgt_ convention; the caller id is honored.
        assert!(stored.iter().any(|w| w.id == "keepme"));
        assert!(stored
            .iter()
            .any(|w| w.id.starts_with("wgt_") && w.id != "keepme"));
        assert!(engine.store.get_widget("old").await.unwrap().is_none());
    }

    #[test]
    fn palette_wire_strings_are_stable() {
        use render::Palette;
        assert_eq!(Palette::Mono.as_str(), "mono");
        assert_eq!(Palette::Rgba.as_str(), "rgba");
        assert_eq!(Palette::Rgb565.as_str(), "rgb565");
    }

    #[test]
    fn device_profile_defaults_and_eink_flag() {
        use render::{DeviceProfile, Palette};
        let desk = DeviceProfile::desk_eink();
        assert_eq!((desk.w, desk.h, desk.bit_depth), (800, 480, 1));
        assert!(desk.is_eink());
        let watch = DeviceProfile::watch_lcd();
        assert_eq!((watch.w, watch.h), (240, 240));
        assert!(!watch.is_eink());
        // A non-mono palette at bit_depth 1 still reads as e-ink (either predicate).
        let odd = DeviceProfile {
