...
mod tests {
    ...
    fn de_bool_flag_is_permissive() {
        for truthy in ["1", "true", "yes", "on"] {
            let q: EventsQuery =
                serde_json::from_value(json!({ "internal": truthy })).unwrap();
    ...
    async fn catalog_lists_kinds_sources_and_endpoints() {
        ...
        assert!(v["widget_kinds"].as_array().unwrap().contains(&json!("stat")));
    ...
    async fn update_widget_patches_and_clears_cache_on_source_change() {
        ...
        assert!(v["widget"].get("last_value").is_none(), "source change clears cache");
    }
    ...
    async fn delete_widget_present_and_missing() {
        ...
        .await;
        let (status, v) =
            delete_widget(State(ctx.clone()), Path(("d1".into(), "w1".into()))).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(v["ok"], json!(true));
        let (status, _) =
            delete_widget(State(ctx), Path(("d1".into(), "w1".into()))).await;
        assert_eq!(status, StatusCode::NOT_FOUND);
    }
    ...
    async fn update_widget_layout_grid_canvas_and_missing() {
        ...
        let (status, v) = update_widget_layout(
            State(ctx.clone()),
            Path(("d1".into(), "w1".into())),
            Json(body(json!({ "canvas": { "x": 9.0, "y": 8.0, "w": 7.0, "h": 6.0 } }))),
        )
        .await;
        assert_eq!(status, StatusCode::OK);
    ...
    async fn refresh_widget_static_returns_value_and_missing_is_404() {
        ...
        // Missing widget ⇒ 404.
        let (status, _) =
            refresh_widget(State(ctx), Path(("d1".into(), "ghost".into()))).await;
        assert_eq!(status, StatusCode::NOT_FOUND);
    }
    ...
    async fn refresh_widget_returns_200_with_error_body_on_source_failure() {
        ...
        .await;
        let (status, v) =
            refresh_widget(State(ctx), Path(("d1".into(), "w1".into()))).await;
        assert_eq!(status, StatusCode::OK);
        assert!(v.get("error").is_some(), "source failure surfaces as error body");
    }
    ...
    async fn device_ensure_defaults_name_to_id() {
        ...
        let dash = ctx.engine.store.get_dashboard(dash_id).await.unwrap().unwrap();
    ...
    async fn set_device_config_handler_updates_and_rejects_bad_widgets() {
        ...
        let (status, v) = read(resp).await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert!(v["error"].as_str().unwrap().contains("not an allowed core_endpoint"));
    }
    ...
    async fn events_query_default_holds_viewer_guard_semantics() {
        ...
        let internal: EventsQuery =
            serde_json::from_value(json!({ "internal": "1" })).unwrap();
