...
mod tests {
    ...
    async fn dashboard_crud_round_trips() {
        ...
        store.upsert_dashboard(&test_dashboard("d1", "Home")).await.unwrap();
        store.upsert_dashboard(&test_dashboard("d2", "Ops")).await.unwrap();
    ...
    async fn update_layout_grid_only_and_canvas_only_are_independent() {
        ...
        let updated = store
            .update_widget_layout("w1", GridLayout { x: 2, y: 3, w: 6, h: 5 })
            .await
            .unwrap()
            .unwrap();
        ...
        let canvas = CanvasLayout { x: 11.0, y: 22.0, w: 33.0, h: 44.0 };
        let updated = store
            .update_widget_layout_fields_for_dashboard("d1", "w1", None, Some(canvas))
            .await
            .unwrap()
            .unwrap();
        assert_eq!((updated.layout.x, updated.layout.h), (2, 5), "grid preserved");
    ...
    async fn upsert_widget_broadcasts_to_subscribers() {
        ...
        match event {
            DashboardEvent::WidgetUpdated { dashboard_id, widget } => {
    ...
    async fn update_widget_value_broadcasts_data_event() {
        ...
        let event = rx.recv().await.unwrap();
        match event {
            DashboardEvent::WidgetData { widget_id, value, .. } => {
